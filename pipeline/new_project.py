...
Creates the full folder and file scaffold:
    <path>/<name>/
...
Projects live outside the repo by default (at --path) so they are never
...
def scaffold_project(project_name: str, projects_dir: Path) -> Path:
    ...
    # Copy template and fill in project name
...
def main() -> None:
    ...
    print(f"  2. Place your video at {project_dir / '1_videos' / '<video_name>.<ext>'}")
    print(f"  3. Run the sweep:")
    print(f"       uv run python pipeline/tune.py --project {project_dir} --video <video_name>")
